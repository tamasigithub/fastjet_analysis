...
void init_eve(int &i)
{
	...
	for(int j = 0; j < trees[i]->GetEntries(); j++)
	{
		...
		gen_4th_bJpT[i][eve_nums[i]] = L3_JpT[i];//CHANGED
...
{	
	...
	if(i < 4)// 2 or 3 (TTT)
	{
		...
		h_trig_4L_bJpT.push_back(new TH1F(h_name, "3^{rd} b-jet p_{T} of the 3L- TTT triggered jet;p_{T, bJ3} [GeV/c];",nbinsMinus, pt_bins));
		sprintf(h_name,"GenTTT_4LbJpT_%d",i%2);
		h_4L_bJpT.push_back(new TH1F(h_name, "3^{rd} b-jet p_{T};p_{T, bJ3} [GeV/c];",nbinsMinus, pt_bins));
		sprintf(h_name,"RatioTTT_4LbJpT_%d",i%2);
		h_4L_bJpT_ratio.push_back(new TH1F(h_name, "fraction of TTT triggered events vs 3^{rd} b-jet p_{T};p_{T, bJ3} [GeV/c];#frac{TTT triggered events}{total events}",nbinsMinus, pt_bins));
		sprintf(h_name,"PhenoFracTTT_4LbJpT_%d",i%2);
		h_4L_bJpT_pheno_frac.push_back(new TH1F(h_name, "fraction of pheno ana events vs 3^{rd} b-jet p_{T} for TTT;p_{T, bJ3} [GeV/c];tot pheno ana events #times fraction of TTT triggered events",nbinsMinus, pt_bins));
	}
	else if (i < 6) // 4 or 5 (Calo)
	{
		pT_threshold = CaloT_for3L;//753e3;//MeV
		sprintf(h_name,"Calo_4LbJpT_%d",i%2);
		h_trig_4L_bJpT.push_back(new TH1F(h_name, "3^{rd} b-jet p_{T} of the 3L- Calo triggered jet;p_{T, bJ3} [GeV/c];",nbinsMinus, pt_bins));
		sprintf(h_name,"GenCal_4LbJpT_%d",i%2);
		h_4L_bJpT.push_back(new TH1F(h_name, "3^{rd} b-jet p_{T};p_{T, bJ3} [GeV/c];",nbinsMinus, pt_bins));
		sprintf(h_name,"RatioCal_4LbJpT_%d",i%2);
		h_4L_bJpT_ratio.push_back(new TH1F(h_name, "fraction of Calo triggered events vs 3^{rd} b-jet p_{T};p_{T, bJ3} [GeV/c];#frac{Calo triggered events}{total events}",nbinsMinus, pt_bins));
		sprintf(h_name,"PhenoFracCal_4LbJpT_%d",i%2);
		h_4L_bJpT_pheno_frac.push_back(new TH1F(h_name, "fraction of pheno ana events vs 3^{rd} b-jet p_{T} for Calo;p_{T, bJ3} [GeV/c];tot pheno ana events #times fraction of Calo triggered events",nbinsMinus, pt_bins));
	}
...
void summary_significance()
{
	...
	//CHANGE
	h_4L_bJpT_pheno.push_back((TH1F*)((TH1F*)f_pheno->Get("Ana_bjet3LPt1"))->Rebin(nbinsMinus, "h4bJL_pheno_sig", pt_bins));
	h_4L_bJpT_pheno.push_back((TH1F*)((TH1F*)f_pheno->Get("Ana_bjet3LPtB"))->Rebin(nbinsMinus, "h4bJL_pheno_bkg", pt_bins));
	...
	h_TTT_Z = new TH1F("h_TTT_Z","Z_{i} vs 3^{rd} b-jet p_{T} for TTT; p_{T, bJ3}[GeV/c];Z_i",nbinsMinus, pt_bins); 
	h_Cal_Z = new TH1F("h_Cal_Z","Z_{i} vs 3^{rd} b-jet p_{T} for Calo; p_{T, bJ3}[GeV/c];Z_i",nbinsMinus, pt_bins);
	h_pheno_Z = new TH1F("h_pheno_Z","Z_{i} vs 3^{rd} b-jet p_{T} for generator level analysis; p_{T, bJ3}[GeV/c];Z_i",nbinsMinus, pt_bins);
