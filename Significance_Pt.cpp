...
const char *output_file_name = "SignificanceVs4thPt_1";
...
//Float_t Significance0[n]   = {0};
//Float_t Significance_1[n]  = {0};
//Float_t Significance_2[n]  = {0};
//Float_t Significance2[n]   = {0};
//Float_t Significance2_5[n] = {0};
//Float_t Significance3[n]   = {0};

Float_t SignificanceGain1[n]   = {0};
Float_t SignificanceGain2[n]   = {0};
//Float_t SignificanceGain0[n]   = {0};
//Float_t SignificanceGain_1[n]  = {0};
//Float_t SignificanceGain_2[n]  = {0};
//Float_t SignificanceGain2[n]   = {0};
//Float_t SignificanceGain2_5[n] = {0};
//Float_t SignificanceGain3[n]   = {0};
...
//TH1D *h4_Significance2   = nullptr;
//TH1D *h4_Significance2_5 = nullptr;
...
void plot_VsPt1()
{
	...
	h4_sig1 = Ana_bjet4LPt1->Rebin(nbinsMinus1, "h4_sig1", pt_bins);
	h4_bg   = Ana_bjet4LPtB->Rebin(nbinsMinus1, "h4_bg", pt_bins);
	...
	h4_Significance1  = new TH1D("h4_Significance1", "Significance Vs 4^{th} jet p_{T} after analysis cuts; p_{T, bJ4} [GeV/c];", nbinsMinus1, pt_bins);
	h4_Significance2  = new TH1D("h4_Significance2", "Significance square Vs 4^{th} jet p_{T} after analysis cuts; p_{T, bJ4} [GeV/c];", nbinsMinus1, pt_bins);
	...
	for(int i = 1; i < n + 1; i++)
	{
		...
		if(nAnaB[i-1] != 0)
		{
			SoverB1[i-1]      = (nAna1[i-1] * norm_signal)/(nAnaB[i-1] * norm_bckgnd);
			Significance1[i-1] = (nAna1[i-1] * norm_signal)/std::sqrt(nAnaB[i-1] * norm_bckgnd);
			Significance2[i-1] = std::pow(nAna1[i-1] * norm_signal, 2)/(nAnaB[i-1] * norm_bckgnd);
		}
		...
		h4_Significance2->SetBinContent(i, Significance2[i-1]);
        	...
        	ofs<<pT_threshold[i-1]<<" "<<nAna1[i-1]<<" "<<nAnaB[i-1]<<" "<<nAna1[i-1]*norm_signal<<" "<<nAnaB[i-1]*norm_bckgnd<<" "<<SoverB1[i-1]<<" "<<Significance1[i-1]<<" "<<Significance2[i-1]<<"\n";
	}
	ofs.close();
...
void plot_graph()
{
	...
	h4_SoverB1->GetYaxis()->SetTitle("S/B");
