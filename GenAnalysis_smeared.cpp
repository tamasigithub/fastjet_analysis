...
void plot()
{
	...
	fB_1  = new TFile(inp_fileB_1,"READ");
	tB.Add(inp_fileB);
	tB.Add(inp_fileB_1);
	...
	double norm_bckgnd = (IntLumi * pp4bXsec)/tot_MCevents_B;
	std::cout<<"Number of signal events used =  " << tot_MCevents <<std::endl;
	std::cout<<"Number of background events used =  " << tot_MCevents_B <<std::endl;
	...
	int T1 = 0, T0 = 0, T_1 = 0, T_2 = 0, T2 = 0, T2_5 = 0, T3 = 0, TB = 0;
	...
	X8lat = X7lat + 0.095;
	X9lat = X8lat + 0.095;
	lat.DrawLatex(X1lat, Ylat, "Cuts, S/#sqrt{B}"); 
	...
	////lat.DrawLatex(X3lat, Ylat, Form("%.1e", (T1*norm_signal1)/(TB*norm_bckgnd)));
	////lat.DrawLatex(X4lat, Ylat, Form("%.1e", (T0*norm_signal0)/(TB*norm_bckgnd)));
	////lat.DrawLatex(X5lat, Ylat, Form("%.1e", (T_1*norm_signal_1)/(TB*norm_bckgnd)));
	////lat.DrawLatex(X6lat, Ylat, Form("%.1e", (T_2*norm_signal_2)/(TB*norm_bckgnd)));
	////lat.DrawLatex(X7lat, Ylat, Form("%.1e", (T2*norm_signal2)/(TB*norm_bckgnd)));
	////lat.DrawLatex(X8lat, Ylat, Form("%.1e", (T2_5*norm_signal2_5)/(TB*norm_bckgnd)));
	////lat.DrawLatex(X9lat, Ylat, Form("%.1e", (T3*norm_signal3)/(TB*norm_bckgnd)));
	...
	////lat.DrawLatex(X1lat, Ylat, "S/B");
	////lat.DrawLatex(X2lat, Ylat, "--");
	////lat.DrawLatex(X3lat, Ylat, Form("%.1e", (n1*norm_signal1)/(nB*norm_bckgnd)));
	////lat.DrawLatex(X4lat, Ylat, Form("%.1e", (n0*norm_signal0)/(nB*norm_bckgnd)));
	////lat.DrawLatex(X5lat, Ylat, Form("%.1e", (n_1*norm_signal_1)/(nB*norm_bckgnd)));
	////lat.DrawLatex(X6lat, Ylat, Form("%.1e", (n_2*norm_signal_2)/(nB*norm_bckgnd)));
	////lat.DrawLatex(X7lat, Ylat, Form("%.1e", (n2*norm_signal2)/(nB*norm_bckgnd)));
	////lat.DrawLatex(X8lat, Ylat, Form("%.1e", (n2_5*norm_signal2_5)/(nB*norm_bckgnd)));
	////lat.DrawLatex(X9lat, Ylat, Form("%.1e", (n3*norm_signal3)/(nB*norm_bckgnd)));
	////
	////Ylat -= step;
	...
	Ylat -= step;
	lat.DrawLatex(X1lat, Ylat, "N_{b-tags} #geq 4");
	...
	////lat.DrawLatex(X1lat, Ylat, "S/B");
	////lat.DrawLatex(X2lat, Ylat, "--");
	////lat.DrawLatex(X3lat, Ylat, Form("%.1e", (nh1*norm_signal1)/(nhB*norm_bckgnd)));
	////lat.DrawLatex(X4lat, Ylat, Form("%.1e", (nh0*norm_signal0)/(nhB*norm_bckgnd)));
	////lat.DrawLatex(X5lat, Ylat, Form("%.1e", (nh_1*norm_signal_1)/(nhB*norm_bckgnd)));
	////lat.DrawLatex(X6lat, Ylat, Form("%.1e", (nh_2*norm_signal_2)/(nhB*norm_bckgnd)));
	////lat.DrawLatex(X7lat, Ylat, Form("%.1e", (nh2*norm_signal2)/(nhB*norm_bckgnd)));
	////lat.DrawLatex(X8lat, Ylat, Form("%.1e", (nh2_5*norm_signal2_5)/(nhB*norm_bckgnd)));
	////lat.DrawLatex(X9lat, Ylat, Form("%.1e", (nh3*norm_signal3)/(nhB*norm_bckgnd)));
	...
	RecMH1H2_B->SetTitle("Di-higgs Invariant Mass");
	RecMH1H2_B->GetXaxis()->SetTitle("M_{rec, H1H2} [GeV]");
	RecMH1H2_B->Scale(1e-3);
	max_rangeM = RecMH1H2_B->GetMaximum() * 1.7;
	RecMH1H2_B->GetYaxis()->SetRangeUser(1e2, max_rangeM);
	RecMH1H2_B->Draw("");
	RecMH1H2__2->Draw(" same");
	RecMH1H2_0->Draw(" same");
	RecMH1H2__1->Draw(" same");
	RecMH1H2_3->Draw(" same");
	RecMH1H2_2->Draw(" same");
	RecMH1H2_2_5->Draw(" same");
	RecMH1H2_1->Draw(" same");
	gPad->Write("c42b");
	...
	RecPtH1H2_B->SetTitle("Transverse Momentum of the di-Higgs system");
	RecPtH1H2_B->GetXaxis()->SetTitle("p_{T, rec, H1H2} [GeV/c]");
	RecPtH1H2_B->Scale(1e-3);
	max_rangePt = RecPtH1H2_B->GetMaximum() * 1.7;
	RecPtH1H2_B->GetYaxis()->SetRangeUser(1e2, max_rangePt);
	//RecPtH1H2_B->GetYaxis()->SetRangeUser(1e2, 0.3e6);
	RecPtH1H2_B->Draw("");
	RecPtH1H2__2->Draw(" same");
	RecPtH1H2_0->Draw(" same");
	RecPtH1H2__1->Draw(" same");
	RecPtH1H2_3->Draw(" same");
	RecPtH1H2_2->Draw(" same");
	RecPtH1H2_2_5->Draw(" same");
	RecPtH1H2_1->Draw(" same");
	C->Update();
	gPad->Write("c42d");
	C->Print(out_file_,"pdf");
	...
	C->Divide(4,2);
	C->cd(1);
	combi_selected__2->SetTitle("Selected Combination, k_{#lambda} = -2");
	combi_selected__2->Draw();
	C->cd(2);
	combi_selected__1->SetTitle("Selected Combination, k_{#lambda} = -1");
	combi_selected__1->Draw();
	C->cd(3);
	combi_selected_0->SetTitle("Selected Combination, k_{#lambda} = 0");
	combi_selected_0->Draw();
	C->cd(4);
	combi_selected_1->SetTitle("Selected Combination, k_{#lambda} = 1");
	combi_selected_1->Draw();
	C->cd(5);
	combi_selected_2->SetTitle("Selected Combination, k_{#lambda} = 2");
	combi_selected_2->Draw();
	C->cd(6);
	combi_selected_2_5->SetTitle("Selected Combination, k_{#lambda} = 2.5");
	combi_selected_2_5->Draw();
	C->cd(7);
	combi_selected_3->SetTitle("Selected Combination, k_{#lambda} = 3");
	combi_selected_3->Draw();
	C->cd(8);
	combi_selected_B->SetTitle("Selected Combination, k_{#lambda} = B");
	...
	Ana_bjetLPt_2->SetTitle("After Ana cuts, k_{#lambda} = -2");
	Ana_bjetLPt_2->Draw();
	C->cd(2);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjetLPt_1->SetTitle("After Ana cuts, k_{#lambda} = -1");
	Ana_bjetLPt_1->Draw();
	C->cd(3);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjetLPt0->SetTitle("After Ana cuts, k_{#lambda} = 0");
	Ana_bjetLPt0->Draw();
	C->cd(4);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjetLPt1->SetTitle("After Ana cuts, k_{#lambda} = 1");
	Ana_bjetLPt1->Draw();
	C->cd(5);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjetLPt2->SetTitle("After Ana cuts, k_{#lambda} = 2");
	Ana_bjetLPt2->Draw();
	C->cd(6);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjetLPt2_5->SetTitle("After Ana cuts, k_{#lambda} = 2.5");
	Ana_bjetLPt2_5->Draw();
	C->cd(7);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjetLPt3->SetTitle("After Ana cuts, k_{#lambda} = 3");
	Ana_bjetLPt3->Draw();
	C->cd(8);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjetLPtB->SetTitle("After Ana cuts, k_{#lambda} = B");
	Ana_bjetLPtB->Draw();
	C->Print(out_file_,"pdf");
	...
	C->cd(1);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPt_2->SetTitle("After Ana cuts, k_{#lambda} = -2");
	Ana_bjet2LPt_2->Draw();
	C->cd(2);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPt_1->SetTitle("After Ana cuts, k_{#lambda} = -1");
	Ana_bjet2LPt_1->Draw();
	C->cd(3);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPt0->SetTitle("After Ana cuts, k_{#lambda} = 0");
	Ana_bjet2LPt0->Draw();
	C->cd(4);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPt1->SetTitle("After Ana cuts, k_{#lambda} = 1");
	Ana_bjet2LPt1->Draw();
	C->cd(5);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPt2->SetTitle("After Ana cuts, k_{#lambda} = 2");
	Ana_bjet2LPt2->Draw();
	C->cd(6);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPt2_5->SetTitle("After Ana cuts, k_{#lambda} = 2.5");
	Ana_bjet2LPt2_5->Draw();
	C->cd(7);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPt3->SetTitle("After Ana cuts, k_{#lambda} = 3");
	Ana_bjet2LPt3->Draw();
	C->cd(8);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPtB->SetTitle("After Ana cuts, k_{#lambda} = B");
	Ana_bjet2LPtB->Draw();
	C->Print(out_file_,"pdf");
	...
	C->cd(1);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet3LPt_2->SetTitle("After Ana cuts, k_{#lambda} = -2");
	Ana_bjet3LPt_2->Draw();
	C->cd(2);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet3LPt_1->SetTitle("After Ana cuts, k_{#lambda} = -1");
	Ana_bjet3LPt_1->Draw();
	C->cd(3);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet3LPt0->SetTitle("After Ana cuts, k_{#lambda} = 0");
	Ana_bjet3LPt0->Draw();
	C->cd(4);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet3LPt1->SetTitle("After Ana cuts, k_{#lambda} = 1");
	Ana_bjet3LPt1->Draw();
	C->cd(5);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet3LPt2->SetTitle("After Ana cuts, k_{#lambda} = 2");
	Ana_bjet3LPt2->Draw();
	C->cd(6);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet3LPt2_5->SetTitle("After Ana cuts, k_{#lambda} = 2.5");
	Ana_bjet3LPt2_5->Draw();
	C->cd(7);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet3LPt3->SetTitle("After Ana cuts, k_{#lambda} = 3");
	Ana_bjet3LPt3->Draw();
	C->cd(8);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet3LPtB->SetTitle("After Ana cuts, k_{#lambda} = B");
	Ana_bjet3LPtB->Draw();
	C->Print(out_file_,"pdf");
	...
	C->cd(1);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet4LPt_2->SetTitle("After Ana cuts, k_{#lambda} = -2");
	Ana_bjet4LPt_2->Draw();
	C->cd(2);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet4LPt_1->SetTitle("After Ana cuts, k_{#lambda} = -1");
	Ana_bjet4LPt_1->Draw();
	C->cd(3);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet4LPt0->SetTitle("After Ana cuts, k_{#lambda} = 0");
	Ana_bjet4LPt0->Draw();
	C->cd(4);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet4LPt1->SetTitle("After Ana cuts, k_{#lambda} = 1");
	Ana_bjet4LPt1->Draw();
	C->cd(5);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet4LPt2->SetTitle("After Ana cuts, k_{#lambda} = 2");
	Ana_bjet4LPt2->Draw();
	C->cd(6);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet4LPt2_5->SetTitle("After Ana cuts, k_{#lambda} = 2.5");
	Ana_bjet4LPt2_5->Draw();
	C->cd(7);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet4LPt3->SetTitle("After Ana cuts, k_{#lambda} = 3");
	Ana_bjet4LPt3->Draw();
	C->cd(8);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet4LPtB->SetTitle("After Ana cuts, k_{#lambda} = B");
	Ana_bjet4LPtB->Draw();
	C->Print(out_file_,"pdf");
	
	//! Scale
	Ana_bjetLPtB->Scale(norm_bckgnd);
	Ana_bjetLPt_2->Scale(norm_signal_2);
	Ana_bjetLPt_1->Scale(norm_signal_1);
	Ana_bjetLPt0->Scale(norm_signal0);
	Ana_bjetLPt1->Scale(norm_signal1);
	Ana_bjetLPt2->Scale(norm_signal2);
	Ana_bjetLPt2_5->Scale(norm_signal2_5);
	Ana_bjetLPt3->Scale(norm_signal3);
	//! Scale
	Ana_bjet2LPtB->Scale(norm_bckgnd);
	Ana_bjet2LPt_2->Scale(norm_signal_2);
	Ana_bjet2LPt_1->Scale(norm_signal_1);
	Ana_bjet2LPt0->Scale(norm_signal0);
	Ana_bjet2LPt1->Scale(norm_signal1);
	Ana_bjet2LPt2->Scale(norm_signal2);
	Ana_bjet2LPt2_5->Scale(norm_signal2_5);
	Ana_bjet2LPt3->Scale(norm_signal3);
	//! Scale
	Ana_bjet3LPtB->Scale(norm_bckgnd);
	Ana_bjet3LPt_2->Scale(norm_signal_2);
	Ana_bjet3LPt_1->Scale(norm_signal_1);
	Ana_bjet3LPt0->Scale(norm_signal0);
	Ana_bjet3LPt1->Scale(norm_signal1);
	Ana_bjet3LPt2->Scale(norm_signal2);
	Ana_bjet3LPt2_5->Scale(norm_signal2_5);
	Ana_bjet3LPt3->Scale(norm_signal3);
	//! Scale
	Ana_bjet4LPtB->Scale(norm_bckgnd);
	Ana_bjet4LPt_2->Scale(norm_signal_2);
	Ana_bjet4LPt_1->Scale(norm_signal_1);
	Ana_bjet4LPt0->Scale(norm_signal0);
	Ana_bjet4LPt1->Scale(norm_signal1);
	Ana_bjet4LPt2->Scale(norm_signal2);
	Ana_bjet4LPt2_5->Scale(norm_signal2_5);
	Ana_bjet4LPt3->Scale(norm_signal3);
	//! page53
	C->Clear();
	C->Divide(2,2);
	C->cd(1);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjetLPt_2->SetTitle("after Ana cuts, normalized to 30ab^{-1}");
	max_rangeM = Ana_bjetLPt_2->GetMaximum() * 1.3;
	Ana_bjetLPt_2->GetYaxis()->SetRangeUser(1e3, max_rangeM);
	Ana_bjetLPt_2->Draw();
	Ana_bjetLPt_1->Draw("same");
	Ana_bjetLPt0->Draw("same");
	Ana_bjetLPt1->Draw("same");
	Ana_bjetLPt2->Draw("same");
	Ana_bjetLPt2_5->Draw("same");
	Ana_bjetLPt3->Draw("same");
	gPad->Write("c53a");
	C->cd(2);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPt_2->SetTitle("after Ana cuts, normalized to 30ab^{-1}");
	max_rangeM = Ana_bjet2LPt_2->GetMaximum() * 1.3;
	Ana_bjet2LPt_2->GetYaxis()->SetRangeUser(1e3, max_rangeM);
	Ana_bjet2LPt_2->Draw();
	Ana_bjet2LPt_1->Draw("same");
	Ana_bjet2LPt0->Draw("same");
	Ana_bjet2LPt1->Draw("same");
	Ana_bjet2LPt2->Draw("same");
	Ana_bjet2LPt2_5->Draw("same");
	Ana_bjet2LPt3->Draw("same");
	gPad->Write("c53b");
	C->cd(3);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet3LPt_2->SetTitle("after Ana cuts, normalized to 30ab^{-1}");
	max_rangeM = Ana_bjet3LPt_2->GetMaximum() * 1.3;
	Ana_bjet3LPt_2->GetYaxis()->SetRangeUser(1e3, max_rangeM);
	Ana_bjet3LPt_2->Draw();
	...
	max_rangeM = Ana_bjet4LPtB->GetMaximum() * 1.3;
...
//	C->Clear();
	...
	C->cd(1);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjetLPtB->SetTitle("after Ana cuts, normalized to 30ab^{-1}");
	max_rangeM = Ana_bjetLPtB->GetMaximum() * 1.3;
	Ana_bjetLPtB->GetYaxis()->SetRangeUser(1e3, max_rangeM);
	Ana_bjetLPtB->Draw();
	Ana_bjetLPt_2->Draw("same");
	Ana_bjetLPt_1->Draw("same");
	Ana_bjetLPt0->Draw("same");
	Ana_bjetLPt1->Draw("same");
	Ana_bjetLPt2->Draw("same");
	...
	C->cd(2);
	gPad->SetLogy(); gPad->SetLogx();
	Ana_bjet2LPtB->SetTitle("after Ana cuts, normalized to 30ab^{-1}");
	max_rangeM = Ana_bjet2LPtB->GetMaximum() * 1.3;
	Ana_bjet2LPtB->GetYaxis()->SetRangeUser(1e3, max_rangeM);
	Ana_bjet2LPtB->Draw();
	Ana_bjet2LPt_2->Draw("same");
	Ana_bjet2LPt_1->Draw("same");
	Ana_bjet2LPt0->Draw("same");
	Ana_bjet2LPt1->Draw("same");
	Ana_bjet2LPt2->Draw("same");
	...
	//! Unscale
	Ana_bjetLPtB->Scale(1/norm_bckgnd);
	Ana_bjetLPt_2->Scale(1/norm_signal_2);
	Ana_bjetLPt_1->Scale(1/norm_signal_1);
	Ana_bjetLPt0->Scale(1/norm_signal0);
	Ana_bjetLPt1->Scale(1/norm_signal1);
	Ana_bjetLPt2->Scale(1/norm_signal2);
	Ana_bjetLPt2_5->Scale(1/norm_signal2_5);
	Ana_bjetLPt3->Scale(1/norm_signal3);
	//! Unscale
	Ana_bjet2LPtB->Scale(1/norm_bckgnd);
	Ana_bjet2LPt_2->Scale(1/norm_signal_2);
	Ana_bjet2LPt_1->Scale(1/norm_signal_1);
	Ana_bjet2LPt0->Scale(1/norm_signal0);
	Ana_bjet2LPt1->Scale(1/norm_signal1);
	Ana_bjet2LPt2->Scale(1/norm_signal2);
	Ana_bjet2LPt2_5->Scale(1/norm_signal2_5);
	Ana_bjet2LPt3->Scale(1/norm_signal3);
	//! Unscale
	Ana_bjet3LPtB->Scale(1/norm_bckgnd);
	Ana_bjet3LPt_2->Scale(1/norm_signal_2);
	Ana_bjet3LPt_1->Scale(1/norm_signal_1);
	Ana_bjet3LPt0->Scale(1/norm_signal0);
	Ana_bjet3LPt1->Scale(1/norm_signal1);
	Ana_bjet3LPt2->Scale(1/norm_signal2);
	Ana_bjet3LPt2_5->Scale(1/norm_signal2_5);
	Ana_bjet3LPt3->Scale(1/norm_signal3);
	//! Unscale
	Ana_bjet4LPtB->Scale(1/norm_bckgnd);
	Ana_bjet4LPt_2->Scale(1/norm_signal_2);
	Ana_bjet4LPt_1->Scale(1/norm_signal_1);
	Ana_bjet4LPt0->Scale(1/norm_signal0);
	Ana_bjet4LPt1->Scale(1/norm_signal1);
	Ana_bjet4LPt2->Scale(1/norm_signal2);
	Ana_bjet4LPt2_5->Scale(1/norm_signal2_5);
	Ana_bjet4LPt3->Scale(1/norm_signal3);
