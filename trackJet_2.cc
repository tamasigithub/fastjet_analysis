...
////////////////aaaa////////////////////////////////////////////////////
  ...
  TFile *f_out = new TFile("./fastjet_output/TriggerStudies_5/EMU5GeV_TRKcorra50TRKbased_PU1kggFhh4b1.0_q1.2GeVeta2.5_30mmR0.4_4_test_2.root","RECREATE");
  //TFile *f_out = new TFile("./fastjet_output/TriggerStudies_5/EMU5GeV_TRKcorra50TRKbased_PU1kMB_q1.2GeVeta2.5_30mmR0.4_4_test_2.root","RECREATE");
  ...
  for(Long64_t i = 0; i < nevents; ++i)
  {
	...
	for (int j = 0; j < nobj; ++j)
	{
		...
		double non_smearedE = std::sqrt(std::pow(pt/sin(theta),2) + std::pow(mass_piPM,2));
		double smearedE = gRandom->Gaus(non_smearedE, SCALEfac_Ereso* std::sqrt(non_smearedE));
		double smearedpT = sin(theta)*std::sqrt(std::pow(smearedE,2) - std::pow(mass_piPM,2));
		if(debug)std::cout<<"E, smeared E: " << non_smearedE << ", " << smearedE <<std::endl;
		if(smearedpT < 0)std::cout<<"pT, smeared pT: " << pt << ", " << smearedpT <<std::endl;
		tjObj.px = smearedpT*cos(phi_Rcalo);
		tjObj.py = smearedpT*sin(phi_Rcalo);
		tjObj.pz = smearedpT/tan(theta);
		tjObj.E  = smearedE;
		//tjObj.px = pt*cos(phi);
		//tjObj.py = pt*sin(phi);
	...
	for(int k = 0; k < trjVec.size(); ++k )
	{
		...
		//***************************************************************//	
		//////////////// DO NOT DO CELL BASED EMULATION //////////////////
		//////////////////////////////////////////////////////////////////
	...
	for(int i2 = 0; i2 < trigger.nbins; i2++)
	{
		...
		if(jetPt.size() >= trigger.Njet_max)
		{
			if(jetPt[4]*1e-3 > trigger.xbins[i2]) trigger.n5b_tot[i2] += 1;
			if(jetPt[3]*1e-3 > trigger.xbins[i2]) trigger.n4b_tot[i2] += 1;
			if(jetPt[2]*1e-3 > trigger.xbins[i2]) trigger.n3b_tot[i2] += 1;
			if(jetPt[1]*1e-3 > trigger.xbins[i2]) trigger.n2b_tot[i2] += 1;
		}
		//! increment n4_tot if there are atleast 4 jets with pt > xbins[i]
		else if (jetPt.size() >= trigger.Njet_max-1)
		{
			if(jetPt[3]*1e-3 > trigger.xbins[i2]) trigger.n4b_tot[i2] += 1;
			if(jetPt[2]*1e-3 > trigger.xbins[i2]) trigger.n3b_tot[i2] += 1;
			if(jetPt[1]*1e-3 > trigger.xbins[i2]) trigger.n2b_tot[i2] += 1;
		...
		else if (jetPt.size() >= trigger.Njet_max-2)
		{
			if(jetPt[2]*1e-3 > trigger.xbins[i2]) trigger.n3b_tot[i2] += 1;
			if(jetPt[1]*1e-3 > trigger.xbins[i2]) trigger.n2b_tot[i2] += 1;
		...
		else if (jetPt.size() >= trigger.Njet_max-3)
		{
			if(jetPt[1]*1e-3 > trigger.xbins[i2]) trigger.n2b_tot[i2] += 1;
