...
int main() 
{
  ...
  for(int i = 0; i < nevents; ++i)
  {
	...
	double ChargedPcle_PtThreshold = 300;//MeV
	...
	for(unsigned ii = 0; ii < incl_bquarks_pt.size(); ii++)
		...
		for (unsigned jj = 0; jj < genOut.jetPt_sm.size(); jj++)
		{
		...
		if(dR < 0.4)
		{
			...
			//! simple push_back should also work here
			genOut.vectorof_bJetsPt.push_back(genOut.jetPt_sm[bestTruthJet]);
