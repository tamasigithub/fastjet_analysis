...
///////////////////////////////////////////////////////////////  
  ...
  //TFile *f_eve = new TFile("/media/tamasi/Z/PhD/fastjet/fastjet_output/TriggerStudies_4/user.tkar.EventList_1_5_allAnaCuts_000001.root","READ");
...
//////////////////////////////////////////////////
  ...
  TFile *f_out = new TFile("./fastjet_output/TriggerStudies_6/Ideal_TrkJPU1kggFhh4b1.5mm_30mm_32211trk1.5_2GeV_97432GeV_5_1.root","RECREATE");
  //TFile *f_out = new TFile("./fastjet_output/TriggerStudies_6/Ideal_TrkJPU1kMB1.5mm_30mm_32211trk1.5_2GeV_97432GeV_3.root","RECREATE");
  ...
  std::vector<double> *x0_rec = 0;
  std::vector<double> *y0_rec = 0;
  ...
  rec.SetBranchAddress("pt", &pt_rec);
  rec.SetBranchAddress("vx", &x0_rec);
  rec.SetBranchAddress("vy", &y0_rec);
  rec.SetBranchAddress("vz", &z0_rec);
  rec.SetBranchAddress("theta", &theta_rec);
  rec.SetBranchAddress("eta", &eta_rec);
  rec.SetBranchAddress("phi", &phi_rec);
  rec.SetBranchAddress("charge", &charge);
  rec.SetBranchAddress("tid", &tid_rec);
  rec.SetBranchAddress("pid", &m_pdg);
  ...
  for(Long64_t i_event = 0; i_event < nevents; ++i_event)
  {
	...
	//{
		...
		for(int ik = 0; ik < pt_rec->size(); ++ik)
		{
			...
			//! smear 1/pT 
			double sigma_invpT =3.549e-3*(1/(*pt_rec)[ik]) + 5.772e-7;//MeV-1
		     	double pT_smeared = 1/gRandom->Gaus(1/(*pt_rec)[ik],sigma_invpT);
			//! smear z0
			double sigma_z0_1 = 2.2*(1e3/pT_smeared) + 0.25;
			double sigma_z0_2 = 0.626 * std::pow((*eta_rec)[ik],2) + 0.001735 * (*eta_rec)[ik] + 0.598;
			double sigma_z0 = std::sqrt(std::pow(sigma_z0_1 ,2) + std::pow(sigma_z0_2 ,2));
		     	double z0_smeared = gRandom->Gaus((*z0_rec)[ik],sigma_z0);
			if(std::fabs(z0_smeared) > 100) continue; 	

			//if(std::fabs((*z0_rec)[ik]) > 100) continue; 	
			if(std::fabs((*x0_rec)[ik]) > 20) continue; 	
			if(std::fabs((*y0_rec)[ik]) > 20) continue; 	
			if(std::abs((*m_pdg)[ik]) == 25 || std::abs((*m_pdg)[ik]) == 5 || std::abs((*m_pdg)[ik])==16) continue; // get rid of the Higgs's and the b quarks
			//if(std::abs((*m_pdg)[ik]) == 11 && std::fabs((*pt_rec)[ik]) < 5e3) continue; // get rid of the Higgs's and the b quarks

			if(std::fabs((*pt_rec)[ik]) < MIN_TRACKPT) continue; 	
			if(std::abs((*charge)[ik]) != 1) continue; 
			//std::cout<<"pT, smeared_pT: " << (*pt_rec)[ik] <<", " <<pT_smeared <<std::endl;
			//std::cout<<"z0, smeared_z0: " << (*z0_rec)[ik] <<", " <<z0_smeared <<std::endl;

			//if(std::abs((*charge)[ik]) == 1) 
			//{
			//	if(std::fabs((*pt_rec)[ik]) < MIN_TRACKPT) continue; 	
			//} 
			//else if ((*charge)[ik] == 0)
			//{
			//	if(std::fabs((*pt_rec)[ik]) < 300) continue; 	
			//}
			if(debug) std::cout <<"pt, eta, z0, charge, pid: " << (*pt_rec)[ik] << ", " << (*eta_rec)[ik] << ", " << (*z0_rec)[ik] << ", " << (*charge)[ik] << ", " << (*m_pdg)[ik] << std::endl;
			pt_recPU.push_back((*pt_rec)[ik]);
			//z0_recPU.push_back((*z0_rec)[ik]);
			z0_recPU.push_back(z0_smeared);
			...
			m_ptPU.push_back((*pt_rec)[ik]);
			m_VzPU.push_back((*z0_rec)[ik]);
			m_thetaPU.push_back((*theta_rec)[ik]);
			m_etaPU.push_back((*eta_rec)[ik]);
			m_phiPU.push_back((*phi_rec)[ik]);
