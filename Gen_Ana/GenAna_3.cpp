...
void Set_higgsPtProps_3()
{
	...
	M_Lhiggs3  = new TH1D("M_Lhiggs3",  "Invariant higgs mass;m_{rec, leading higgs} [GeV];", nbins, MLHmin, MLHmax);
	M_NLhiggs3  = new TH1D("M_NLhiggs3",  "Invariant higgs mass;m_{rec, sub leading higgs} [GeV];", nbins, MLHmin, MLHmax);
	
	//! reconstructed Inv mass of the di-higgs system
	RecMH1H2_3  = new TH1D("RecMH1H2_3",  "Invariant di-higgs mass;m_{truth, h1,h2} [GeV];", nbins, Mmin1, Mmax);
...
void Draw_DiJetInvariantMass_3(int &nb3, int &nhiggs)
{
	v3_bjetPt.clear();
	v3_bjetEta.clear();
	v3_bjetPhi.clear();
	...
	if(v3_bjetPt.size() > 3)
	{
		if(v3_bjetPt[0]*1e-3 >= minLeadingPt  && v3_bjetPt[1]*1e-3 >= minSubLeadingPt  && v3_bjetPt[2]*1e-3 >= third_Pt_cut && v3_bjetPt[3]*1e-3 >= default_Pt_cut )
		{
			nb3++;
